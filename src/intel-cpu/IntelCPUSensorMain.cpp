...
std::optional<uint64_t> getPeciDeviceNum(
    const std::filesystem::path& peciAdapterNamePath)
{
    std::filesystem::path::iterator it = peciAdapterNamePath.begin();
...
{
    std::vector<std::filesystem::path> peciAdapterNamePaths;
    if (!findFiles(std::filesystem::path(peciDevPath), R"(peci-\d+/name$)",
    ...
    for (const std::filesystem::path& peciAdapterNamePath :
         peciAdapterNamePaths)
    {
